...
int search(int data, struct node *tmp, int count)
{
	if(!tmp)
		return -1;
	while(tmp->data!=data)
	{
		search(data,tmp->left,count*2);
		if(tmp->data==data)
			return count;
		search(data,tmp->right,count*2+1);
		if(tmp->data==data)
			return count;
	}
	return -1;
...
int main()
{
	...
	while(choice)
	{
		printf("\n\t\tMENU\n1:Insert\t2:Delete\t3:Print \t4:Print by level\t5:Depth\t\t6:Search\t0:Exit\n");
		scanf("%d",&choice);
		switch(choice)
		{
			case 1:
...
/*			case 2:
			...
			case 6:
				printf("Enter the number : ");
				scanf("%d",&data);
				ret=search(data,root,1);
				if(ret==-1)
					printf("Didn't Found.\n");
				else
					printf("Got at position : %d",ret);
				break;
			case 0:
				printf("\n\n\t\t\t\t\tThanx...\n");
				return 0;
